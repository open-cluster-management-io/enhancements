...
Each registration configuration item is handled by a separate driver instance. Multiple registration items can share the same registration type (e.g., multiple `csr` type registrations). The token driver handles only the `kubeClient` type registration.
...
- ManagedClusterAddOn resource is created on hub by the common add-on manager or administrator
...
| Cluster Driver | Cluster Credential API | Add-on kubeClient Credential API | Add-on csr Type Credential API |
|----------------|------------------------|----------------------------------|--------------------------------|
